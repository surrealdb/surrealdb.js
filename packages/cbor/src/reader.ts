...
const FLOAT16_EXP_LUT = Array.from({ length: 31 }, (_, e) => 2 ** (e - F16_EXP_BIAS));
...
export class Reader {
    ...
    readFloat32(): number {
        const pos = this._pos;
        if (this._byte.length - pos < 4)
            throw new CborRangeError("Tried to read 4 bytes for float32 beyond buffer bounds");
    ...
    readFloat64(): number {
        const pos = this._pos;
        if (this._byte.length - pos < 8)
            throw new CborRangeError("Tried to read 8 bytes for float64 beyond buffer bounds");
