...
/**
 * The pagination block every Agent Memory list surface returns beside its rows.
 *
...
export async function* walkPages<K extends string, T>(
    ...
    for (;;) {
        ...
        if (seen.has(next)) {
            throw new Error(
                `Agent Memory returned a repeated pagination cursor while walking \`${collection}\`; the page walk cannot advance.`,
            );
        }
        seen.add(next);
        cursor = next;
