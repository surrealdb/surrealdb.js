...
export class ConnectionController implements SurrealProtocol, EventPublisher<ConnectionEvents> {
	...
	exportSql(options: SqlExportOptions): Promise<string> {
		if (!this.#engine) throw new ConnectionUnavailable();
	...
	query(query: string, bindings?: Record<string, unknown>, txn?: Uuid): Promise<ReadableStream<QueryResponse>> {
		if (!this.#engine) throw new ConnectionUnavailable();
		return this.#engine.query(query, bindings, txn);
	}

	liveQuery(query: string, bindings?: Record<string, unknown>): Promise<ReadableStream<LiveMessage>> {
		if (!this.#engine) throw new ConnectionUnavailable();
	...
	private onLiveMessage(msg: LiveMessage): void {
		if (msg.action === "KILLED") {
			...
			this.#livePublisher.publish(msg.queryId.toString(), msg.action, msg.value, msg.recordId);
		}
	}
