...
import type { ConnectionState, DriverContext, EngineEvents, SurrealEngine } from "../types/surreal";
...
export class HttpEngine implements SurrealEngine {
    ...
    async export(options?: Partial<ExportOptions>): Promise<string> {
        ...
        const buffer = await postEndpoint(this.#context, this.#state, options ?? {}, endpoint, {
            Accept: "plain/text",
        });
    ...
    async send<Method extends string, Params extends unknown[] | undefined, Result>(
