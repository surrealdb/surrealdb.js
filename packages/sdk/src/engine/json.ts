...
import type { AccessRecordAuth, AnyAuth, AuthResponse, ConnectionState, DriverContext, LiveMessage, MlExportOptions, NamespaceDatabase, QueryResponse, QueryResult, RpcRequest, SqlExportOptions, SurrealProtocol, Token, VersionInfo } from "../types";
...
export abstract class JsonEngine implements SurrealProtocol {
	...
	async use(what: NamespaceDatabase): Promise<NamespaceDatabase> {
		await this.send({
			method: "use",
			params: [what]
		});
	...
	liveQuery(_query: string, _bindings?: Record<string, unknown>): Promise<ReadableStream<LiveMessage>> {
